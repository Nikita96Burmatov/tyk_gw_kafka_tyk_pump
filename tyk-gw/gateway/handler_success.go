...
import (
	"bytes"
	"encoding/base64"
	"github.com/TykTechnologies/tyk/internal/bxais"
	"io"
	"io/ioutil"
	"net/http"
	"slices"
...
func (s *SuccessHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) *http.Response {
	...
	log.Debug("s.Spec.Proxy.ListenPath: ", s.Spec.Proxy.ListenPath)

	var resp ProxyResponse

	// Проверка на входящий запрос:
	// - Если запрос на наши конфиги
	// - Если в запросе не задан заголовок X-Tyk-Recipient
	//   (если задан то запрос должен уйти в target_url)
	if slices.Contains(s.Spec.GlobalConfig.Kafka.ListenPaths, s.Spec.Proxy.ListenPath) {

		// Заголовок, через который передается система куда должен уйти запрос,
		// Если он не задан, перенаправим запрос в брокер сообщений
		recipient := r.Header.Get(s.Spec.GlobalConfig.Kafka.Headers["recipient"])
		_, _ = io.ReadAll(r.Body)

		if recipient == "" {
			r.URL.Path = s.Spec.Proxy.ListenPath + r.URL.Path
			// todo: обезличить
			go bxais.HandleRequest(*r)

			log.Debug("s.Spec.Proxy.ListenPath: ", s.Spec.Proxy.ListenPath)

			w.Header().Set(header.ContentType, header.ApplicationJSON)
			w.WriteHeader(http.StatusOK)
			w.Write([]byte("{\"status\": \"success\",\"message\": \"the message is received\"}"))

			resp = ProxyResponse{
				Response:        r.Response,
				UpstreamLatency: time.Until(t1),
			}
		} else {

			// Если в запросе задан заголовок X-Tyk-Recipient:
			// - Исправим путь на REST путь соответсвуюшей системы
			// - Перенаправляем запрос в proxy target_url

			//systemPath := r.Header.Get("X-Tyk-Path")
			//
			//if systemPath != "" {
			//	r.URL.Path = systemPath
			//}
